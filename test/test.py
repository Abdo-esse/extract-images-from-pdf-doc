...
while i < len(clean_lines):
    ...
    # Détection d'un nom en MAJUSCULES
...
def safe_filename(name):
    """Supprime accents + remplace caractères interdits Windows."""
...
for idx, player in enumerate(players):
    ...
    if idx < len(image_paths):
        ...
        if os.path.exists(src):
            print(f"➡️ Association photo pour {player['prenom']} {player['nom']}")
            filename = f"{safe_filename(player['nom'])}_{safe_filename(player['prenom'])}.png"
            dst = os.path.join(output_named_photos, filename)

            try:
                os.rename(src, dst)
                player["photo"] = dst
            except Exception as e:
                print(f"⚠️ Erreur renommage {src} → {dst} : {e}")
                player["photo"] = ""
        else:
            print(f"⚠️ Image introuvable : {src}")
            player["photo"] = ""
    else:
        player["photo"] = ""
...
print(f"📁 Photos nommées : {output_named_photos}")
